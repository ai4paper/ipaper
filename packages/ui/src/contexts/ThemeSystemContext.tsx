...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __IPAPER_VSCODE_THEME__?: Theme }).__IPAPER_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('ipaper:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('ipaper:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __ipaperApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__ipaperApplyThemeSync = applyIncomingThemeSync;

    return () => {
      if (scopedWindow.__ipaperApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__ipaperApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'ipaper:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('ipaper:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('ipaper:settings-synced', handleSettingsSynced);
  }, []);
