...
const MODELS_DEV_PROXY_URL = "/api/ipaper/models-metadata";
...
const fetchIPaperDefaults = async (): Promise<IPaperDefaults> => {
    try {
...
interface ConfigStore {
    ...
    // IPaper settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and IPaper settings in parallel
                            ...
                            // 1. Check IPaper settings for default agent
                            if (openChamberDefaults.defaultAgent) {
                             ...
                             // 1. Check IPaper settings for default model
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isIPaperCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewIPaperSession(currentSessionId, agents);
                            }
                        }
                    }
