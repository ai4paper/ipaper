...
const getVSCodeWorkspaceProject = (): { projects: ProjectEntry[]; activeProjectId: string | null } | null => {
  ...
  const runtimeApis = (window as unknown as { __IPAPER_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } })
    .__IPAPER_RUNTIME_APIS__;
  ...
  if (streamDebugEnabled()) {
    console.log('[IPaper][VSCode][projects] Using workspace fallback project', entry);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('ipaper:settings-synced', (event: Event) => {
