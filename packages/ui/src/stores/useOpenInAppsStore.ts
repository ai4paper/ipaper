...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('ipaper:settings-synced', settingsHandler);
    window.addEventListener('ipaper:app-ready', appReadyHandler);
    window.addEventListener('ipaper:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __ipaperAppReady?: boolean }).__ipaperAppReady;
    if (appReady) {
      void loadInstalledApps();
    }
