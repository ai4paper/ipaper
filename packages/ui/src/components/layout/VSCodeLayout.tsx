...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __IPAPER_CONNECTION__?: { status?: string } }).__IPAPER_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('ipaper.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __IPAPER_CONNECTION__?: { status?: string } }).__IPAPER_CONNECTION__?.status
    ...
    window.addEventListener('ipaper:connection-status', handler as EventListener);
    return () => window.removeEventListener('ipaper:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ipaper:navigate', handler as EventListener);
    return () => window.removeEventListener('ipaper:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('ipaper_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[IPaper][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[IPaper][VSCode][bootstrap] post-load', {
