...
const GITHUB_RELEASES_URL = 'https://github.com/ai4paper/ipaper/releases';
...
async function installWebUpdate(): Promise<InstallWebUpdateResult> {
  try {
    const response = await fetch('/api/ipaper/update-install', {
      method: 'POST',
...
async function waitForUpdateApplied(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const response = await fetch('/api/ipaper/update-check', {
        method: 'GET',
        headers: { Accept: 'application/json' },
      });
      if (response.ok) {
...
export const UpdateDialog: React.FC<UpdateDialogProps> = ({
  ...
  const updateCommand = info?.updateCommand || 'ipaper update';
  ...
  const handleWebUpdate = useCallback(async () => {
    ...
    if (applied) {
      ...
      setWebUpdateState('error');
      setWebError('Update is taking longer than expected. Wait a bit and refresh, or run: ipaper update');
    }
  }, [info?.currentVersion]);
