...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-ipaper-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-ipaper-file-link="true"]';
  ...
  React.useEffect(() => {
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        if (!resolved) {
          candidate.removeAttribute('data-ipaper-file-link');
          candidate.removeAttribute('data-ipaper-file-ref');
          candidate.removeAttribute('data-ipaper-file-path');
        ...
        candidate.setAttribute('data-ipaper-file-link', 'true');
        candidate.setAttribute('data-ipaper-file-ref', rawCandidate);
        candidate.setAttribute('data-ipaper-file-path', resolved.resolvedPath);
    ...
    const openFileReference = (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-ipaper-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-ipaper-file-link') !== 'true') {
        return;
      }
