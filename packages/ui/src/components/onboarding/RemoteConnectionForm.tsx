...
function getProbeStatusMessage(status: ProbeStatus): string | null {
  switch (status) {
    ...
    case 'wrong-service':
      return 'Server responded but is not running IPaper. Verify the address points to an IPaper server.';
    case 'unreachable':
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="space-y-2 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            {isRecoveryMode
              ? 'Enter the address of an IPaper server to connect to.'
              : 'Enter the address of an IPaper server to connect to.'}
          </p>
        </div>
          ...
          >
            ...
            <div className="text-xs opacity-80">
              {probeResult.status === 'unreachable'
                ...
                : 'Suggestions: Verify the URL points to an IPaper server, or contact the server administrator.'}
            </div>
          </div>
        )}
