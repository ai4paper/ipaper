...
const DEFAULT_PWA_INSTALL_NAME = 'IPaper - AI Coding Assistant';

type PwaInstallNameWindow = Window & {
    __IPAPER_SET_PWA_INSTALL_NAME__?: (value: string) => string;
    __IPAPER_UPDATE_PWA_MANIFEST__?: () => void;
};
...
interface IPaperVisualSettingsProps {
    /** Which settings to show. If undefined, shows all. */
    visibleSettings?: VisibleSetting[];
}

export const IPaperVisualSettings: React.FC<IPaperVisualSettingsProps> = ({ visibleSettings }) => {
    ...
    const applyPwaInstallName = React.useCallback(async (value: string) => {
        ...
        if (typeof win.__IPAPER_SET_PWA_INSTALL_NAME__ === 'function') {
            const resolved = win.__IPAPER_SET_PWA_INSTALL_NAME__(persistedValue);
            setPwaInstallName(resolved);
            return;
        }

        setPwaInstallName(persistedValue || DEFAULT_PWA_INSTALL_NAME);
        win.__IPAPER_UPDATE_PWA_MANIFEST__?.();
    }, []);
                            ...
                            <div className="flex items-center gap-2 py-1.5">
                                ...
                                <Tooltip delayDuration={700}>
                                    ...
                                    <TooltipContent sideOffset={8}>
                                        Import custom themes from ~/.config/ipaper/themes/
                                    </TooltipContent>
                                </Tooltip>
                            </div>
