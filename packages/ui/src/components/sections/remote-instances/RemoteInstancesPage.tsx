...
const phaseLabel = (phase?: string): string => {
  switch (phase) {
    ...
    case 'installing':
      return 'Installing IPaper';
    case 'updating':
      return 'Updating IPaper';
...
export const RemoteInstancesPage: React.FC = () => {
  ...
  const handleSave = React.useCallback(async () => {
    ...
    if (
      normalized.auth.ipaperPassword?.enabled &&
      normalized.auth.ipaperPassword.value?.trim() &&
      normalized.auth.ipaperPassword.store !== 'settings'
    ) {
      const store = window.confirm('Store IPaper UI password in settings.json as plaintext?');
      normalized.auth.ipaperPassword.store = store ? 'settings' : 'never';
      if (!store) {
        normalized.auth.ipaperPassword.value = undefined;
      }
    }
      ...
      <SettingsPageLayout>
        <div className="mb-8">
          <div className="mb-1 px-1 space-y-0.5">
            ...
            <p className="typography-meta text-muted-foreground">Manage SSH-backed IPaper instances.</p>
          </div>
    ...
    <SettingsPageLayout>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          ...
          <p className="typography-meta text-muted-foreground">How IPaper is discovered or started on the remote machine.</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                label="Mode"
                hint="Managed installs/updates and starts IPaper remotely. External assumes it is already running."
              />
            </div>
            <Select
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                ...
                hint="Port IPaper should use on the remote host. Leave empty to let the runtime choose."
              />
            </div>
            <NumberInput
              containerClassName="w-fit"
            ...
            <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
              <div className="w-56 shrink-0">
                <HintLabel
                  ...
                  hint="How IPaper gets installed/updated remotely when mode is Managed."
                />
              </div>
              <Select
            ...
            <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
              <div className="w-56 shrink-0">
                <HintLabel
                  ...
                  hint="If enabled, IPaper daemon is left running remotely when you disconnect."
                />
              </div>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          ...
          <p className="typography-meta text-muted-foreground">Primary local URL that points to the remote IPaper server.</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                ...
                hint="Preferred local port for the main IPaper tunnel. Leave empty for auto-select."
              />
            </div>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        ...
        <section className="px-2 pb-2 pt-0 space-y-3">
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">IPaper UI password (optional)</span>
            <Input
              className="h-7 md:max-w-sm"
              type="password"
              value={draft.auth.ipaperPassword?.value || ''}
              onChange={(event) =>
                updateDraft((current) => ({
                  ...current,
                  auth: {
                    ...
                    ipaperPassword: {
                      enabled: event.target.value.trim().length > 0,
                      value: event.target.value,
                      store: current.auth.ipaperPassword?.store || 'never',
                    },
                  },
                }))
              }
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          ...
          <p className="typography-meta text-muted-foreground">Optional extra SSH forwards in addition to the primary IPaper tunnel.</p>
        </div>
