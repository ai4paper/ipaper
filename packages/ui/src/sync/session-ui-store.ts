...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsIPaperCreated: (sessionId: string) => void
  isIPaperCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewIPaperSession: (sessionId: string, agents: unknown[]) => void
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsIPaperCreated: (sessionId) =>
    set((s) => {
  ...
  isIPaperCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  sendMessage: async (
    ...
    if (draft?.open) {
      ...
      get().initializeNewIPaperSession(created.id, configState.agents ?? [])
