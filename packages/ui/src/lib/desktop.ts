...
export const isDesktopLocalOriginActive = (): boolean => {
  if (typeof window === 'undefined') return false;
  const local = typeof window.__IPAPER_LOCAL_ORIGIN__ === 'string' ? window.__IPAPER_LOCAL_ORIGIN__ : '';
...
export const isDesktopShell = (): boolean => {
  if (typeof window === 'undefined') return false;
  if (typeof window.__IPAPER_LOCAL_ORIGIN__ === 'string' && window.__IPAPER_LOCAL_ORIGIN__.length > 0) {
    return true;
  }
  return isTauriShell();
};
...
export const isVSCodeRuntime = (): boolean => {
  if (typeof window === "undefined") return false;
  const apis = (window as { __IPAPER_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } }).__IPAPER_RUNTIME_APIS__;
  return apis?.runtime?.isVSCode === true;
};

export const isWebRuntime = (): boolean => {
  if (typeof window === "undefined") return false;
  const apis = (window as { __IPAPER_RUNTIME_APIS__?: { runtime?: { platform?: string } } }).__IPAPER_RUNTIME_APIS__;
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__IPAPER_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && tauri?.event?.listen) {
      unlisten = await tauri.event.listen('ipaper:update-progress', (evt) => {
