...
type IPaperEvent = ScheduledTaskRanEvent;
type Listener = (event: IPaperEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'ipaper:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type !== 'ipaper:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource('/api/ipaper/events');
