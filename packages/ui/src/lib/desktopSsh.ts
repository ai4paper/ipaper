...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    ipaperPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const ipaperPassword = parseStoredSecret(authRaw.ipaperPassword || authRaw.ipaper_password);
...
export const listenDesktopSshStatus = async (
  ...
  const unlisten = await listen('ipaper:ssh-instance-status', (event) => {
