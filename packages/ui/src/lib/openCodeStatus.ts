...
export const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const openChamberHealth: IPaperHealthSnapshot | null = await (async () => {
    ...
    try {
      ...
      return json as IPaperHealthSnapshot;
    } catch {
      return null;
    } finally {
      clearTimeout(timeout);
  ...
  const openChamberOpencodeResolutionResult: {
    data: IPaperOpencodeResolution | null;
    status: number | null;
    error: string | null;
  } = await (async () => {
    if (!origin) return null;
    ...
    try {
      ...
      return { data: json as IPaperOpencodeResolution, status: resp.status, error: null };
    } catch (error) {
      return {
        data: null,
        status: null,
  ...
  lines.push(`IPaper version: ${appVersion}`);
