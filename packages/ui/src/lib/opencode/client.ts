...
const resolveDesktopBaseUrl = (): string | null => {
  ...
  const desktopServer = (window as typeof window & {
    __IPAPER_DESKTOP_SERVER__?: { origin: string; apiPrefix?: string };
    __IPAPER_RUNTIME_APIS__?: RuntimeAPIs;
  }).__IPAPER_DESKTOP_SERVER__;

  const isDesktop = Boolean(
    (window as typeof window & { __IPAPER_RUNTIME_APIS__?: RuntimeAPIs }).__IPAPER_RUNTIME_APIS__?.runtime?.isDesktop
  );
...
class OpencodeService {
  ...
  async checkHealth(): Promise<boolean> {
    try {
      ...
      // Check if the upstream API is ready (not just IPaper server)
