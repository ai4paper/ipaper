...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (settings.homeDirectory) {
    ...
    window.__IPAPER_HOME__ = settings.homeDirectory;
  }
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('ipaper.pwaName', normalized);
    } else {
      localStorage.removeItem('ipaper.pwaName');
    }
  }
};
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = (settings: DesktopSettings) => {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent<DesktopSettings>('ipaper:settings-synced', { detail: settings }));
    }
  };
