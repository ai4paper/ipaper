...
import { registerIPaperRoutes } from './lib/opencode/ipaper-routes.js';
...
const DESKTOP_NOTIFY_PREFIX = '[IPaperDesktopNotify] ';
const uiNotificationClients = new Set();
const uiIPaperEventClients = new Set();
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: IPAPER_USER_THEMES_DIR,
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: IPAPER_PROJECTS_CONFIG_DIR,
});
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.IPAPER_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.IPAPER_DESKTOP_NOTIFY === 'true') {
    return true;
  }

  if (process.env.IPAPER_RUNTIME === 'desktop') {
    return true;
  }

  const argv0 = typeof process.argv?.[0] === 'string' ? process.argv[0] : '';
  const argv1 = typeof process.argv?.[1] === 'string' ? process.argv[1] : '';
  return /ipaper-server/i.test(argv0) || /ipaper-server/i.test(argv1);
})();
const ENV_CONFIGURED_OPENCODE_WSL_DISTRO =
  typeof process.env.OPENCODE_WSL_DISTRO === 'string' && process.env.OPENCODE_WSL_DISTRO.trim().length > 0
    ...
    : (
      typeof process.env.IPAPER_OPENCODE_WSL_DISTRO === 'string' &&
      process.env.IPAPER_OPENCODE_WSL_DISTRO.trim().length > 0
        ? process.env.IPAPER_OPENCODE_WSL_DISTRO.trim()
        : null
    );
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.IPAPER_API_PREFIX || ''
);
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiIPaperEventClients) {
      try {
        writeSseEvent(client, {
          type: 'ipaper:scheduled-task-ran',
          properties: {
        ...
        uiIPaperEventClients.delete(client);
      }
    }
  },
  logger: console,
...
async function main(options = {}) {
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    process,
    ipaperVersion: IPAPER_VERSION,
    runtimeName: process.env.IPAPER_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    ipaperDataDir: IPAPER_DATA_DIR,
    ipaperUserConfigRoot: IPAPER_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    projectConfigRuntime,
    scheduledTasksRuntime,
    getIPaperEventClients: () => uiIPaperEventClients,
    writeSseEvent,
  });
