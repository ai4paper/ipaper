...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/ipaper/scheduled-tasks/status', async (_req, res) => {
    try {
      const settings = await readSettingsFromDiskMigrated();
      const projects = sanitizeProjects(settings?.projects || []);
  ...
  app.get('/api/ipaper/events', (req, res) => {
    res.setHeader('Content-Type', 'text/event-stream; charset=utf-8');
    res.setHeader('Cache-Control', 'no-cache, no-transform');
    res.setHeader('Connection', 'keep-alive');
    res.flushHeaders?.();

    const clients = getIPaperEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'ipaper:event-stream-ready',
        properties: {
