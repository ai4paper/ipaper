...
export const normalizeCustomOpenAIBaseURL = (value) => {
  ...
  const allowRemote = isEnvFlagEnabled(process.env.IPAPER_ALLOW_REMOTE_OPENAI_COMPAT_URLS);
  if (!allowRemote && !isAllowedLocalHost(parsed.hostname)) {
    return {
      error: 'Remote custom server URLs are disabled. Set IPAPER_ALLOW_REMOTE_OPENAI_COMPAT_URLS=true to allow this host.',
    };
  }
