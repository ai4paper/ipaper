...
const PACKAGE_NAME = '@ai4paper/ipaper';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/ai4paper/ipaper/main/CHANGELOG.md';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
const UPDATE_CHECK_URL = process.env.IPAPER_UPDATE_API_URL || 'https://api.ipaper.dev/v1/update/check';

function getIPaperConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'ipaper');
  }
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getIPaperConfigDir();
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'ipaper.cmd' : 'ipaper';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.IPAPER_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('ipaper');
  } catch {
    return false;
  }
}
