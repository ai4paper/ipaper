export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/ipaper/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/ipaper/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/ipaper/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/ipaper/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/ipaper/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/ipaper/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/ipaper/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/ipaper/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();
