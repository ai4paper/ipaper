...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveWslExecutablePath = () => {
    ...
    const explicit = [process.env.WSL_BINARY, process.env.IPAPER_WSL_BINARY]
  ...
  const resolveOpencodeCliPath = () => {
    const explicit = [
      ...
      process.env.IPAPER_OPENCODE_PATH,
      process.env.IPAPER_OPENCODE_BIN,
    ]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.IPAPER_NODE_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.IPAPER_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.IPAPER_GIT_BINARY]
