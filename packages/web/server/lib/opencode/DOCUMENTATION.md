...
## Public exports (ipaper-routes.js)
- `registerIPaperRoutes(app, dependencies)`: registers IPaper endpoints:
  - `GET /api/ipaper/update-check`
  - `POST /api/ipaper/update-install`
  - `GET /api/ipaper/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `IPAPER_RATE_LIMIT_MAX_ATTEMPTS`, `IPAPER_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
