export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.IPAPER_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.IPAPER_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.IPAPER_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.IPAPER_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.IPAPER_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.IPAPER_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.IPAPER_TUNNEL_HOSTNAME || undefined;
