export const registerIPaperRoutes = (app, dependencies) => {
  const {
    fs,
    os,
    path,
  ...
  app.get('/api/ipaper/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/ipaper/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(tmpDir, `ipaper-${currentPort}.json`);
      ...
      let restartCmdFallback = `ipaper serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(ipaperDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== IPaper update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting IPaper...
              ${restartCmd}
            ) else (
              echo Update failed
              exit /b 1
  ...
  app.get('/api/ipaper/models-metadata', async (_req, res) => {
    const now = Date.now();
