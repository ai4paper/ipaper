...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.IPAPER_FS_EXEC_TIMEOUT_MS);
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, ipaperUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, ipaperUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(ipaperUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, ipaperUserConfigRoot }) => {
