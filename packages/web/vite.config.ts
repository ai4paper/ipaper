...
const pwaDevEnabled = process.env.IPAPER_DISABLE_PWA_DEV !== '1';
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@ai4paper/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.IPAPER_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.IPAPER_PORT || 3001}`,
  ...
  build: {
    ...
    rollupOptions: {
      external: ['node:child_process', 'node:fs', 'node:path', 'node:url', /^@tauri-apps\//],
      output: {
        manualChunks(id) {
