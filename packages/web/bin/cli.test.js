...
describe('cli entry detection', () => {
  const modulePath = '/tmp/ipaper/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/ipaper';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/ipaper';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'ipaper')).toBe(true);
  });
