...
function resolveApiHost() {
  const configured = typeof process.env.IPAPER_HOST === 'string'
...
function formatUnsafePortWarning(port) {
  return `Port ${port} is browser-unsafe (ERR_UNSAFE_PORT) and is not supported for IPaper UI at ${buildLocalUrl(port, '/')}.`;
}
...
function buildTunnelStartReplayCommand({
  ...
  const parts = ['ipaper', 'tunnel', 'start'];
...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const options = {
    port: DEFAULT_PORT,
    host: undefined,
    uiPassword: process.env.IPAPER_UI_PASSWORD || undefined,
    json: false,
    all: false,
    follow: true,
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'daemon':
      case 'd':
        removedFlagErrors.push('`--daemon` was removed. IPaper now always runs in daemon mode.');
        break;
      case 'try-cf-tunnel':
        removedFlagErrors.push('`--try-cf-tunnel` was removed. Use: ipaper tunnel start --provider cloudflare --mode quick');
        break;
      case 'tunnel-qr':
        removedFlagErrors.push('`--tunnel-qr` was removed. Use: ipaper tunnel start ... --qr');
        break;
      ...
      case 'tunnel':
        removedFlagErrors.push(`\`--${name}\` was removed from top-level serve flow. Use: ipaper tunnel start ...`);
        break;
      default:
...
function showHelp() {
  console.log(`
 IPaper - Web interface for the OpenCode AI coding agent

USAGE:
  ipaper [COMMAND] [OPTIONS]
...
ENVIRONMENT:
  IPAPER_HOST             Bind address (e.g. 0.0.0.0 for all interfaces)
  IPAPER_UI_PASSWORD      Alternative to --ui-password flag
  IPAPER_DATA_DIR         Override IPaper data directory
  ...
  IPAPER_OPENCODE_HOSTNAME  Bind hostname for managed OpenCode server (default: 127.0.0.1)

EXAMPLES:
  ipaper                    # Start in daemon mode on default port 3000 (or free port)
  ipaper --port 8080        # Start on port 8080 (daemon)
  ipaper serve --foreground # Start in foreground (for systemd Type=simple)
  ipaper tunnel help        # Show tunnel lifecycle help
  ipaper logs               # Follow logs for latest running instance
`);
}
...
USAGE:
  ipaper tunnel <SUBCOMMAND> [OPTIONS]
...
COMMON OPTIONS:
  -p, --port              Target IPaper instance port
...
BEHAVIOR NOTES:
  - One active tunnel per IPaper instance.
...
PROFILE USAGE:
  ipaper tunnel profile list [--provider <id>] [--json]
  ipaper tunnel profile show --name <name> [--provider <id>] [--json]
  ipaper tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token <token> [--force] [--json]
  ipaper tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token-file <path> [--force] [--json]
  ipaper tunnel profile remove --name <name> [--provider <id>] [--json]

SHELL COMPLETION:
  ipaper tunnel completion bash   Generate Bash completion script
  ipaper tunnel completion zsh    Generate Zsh completion script
  ipaper tunnel completion fish   Generate Fish completion script

EXAMPLES:
  ipaper tunnel providers
  ipaper tunnel ready --provider cloudflare
  ipaper tunnel doctor --provider cloudflare
  ipaper tunnel status
  ipaper tunnel start --qr
  ipaper tunnel start --profile prod-main
  ipaper tunnel start --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  ipaper tunnel start --provider cloudflare --mode managed-local --config ~/.cloudflared/config.yml
  ipaper tunnel start --dry-run --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  echo "$TOKEN" | ipaper tunnel profile add --provider cloudflare --mode managed-remote --name prod-main --hostname app.example.com --token-stdin
  ipaper tunnel profile list --provider cloudflare
  ipaper tunnel profile list --json --show-secrets
  ipaper tunnel stop --port 3000
`);
}
...
function generateCompletionScript(shell) {
  ...
  if (normalized === 'bash') {
    return `# Bash completion for ipaper tunnel
# Add to ~/.bashrc: eval "$(ipaper tunnel completion bash)"
_ipaper_tunnel() {
...
complete -F _ipaper_tunnel ipaper
`;
  }

  if (normalized === 'zsh') {
    return `#compdef ipaper
# Zsh completion for ipaper tunnel
# Add to ~/.zshrc: eval "$(ipaper tunnel completion zsh)"
...
compdef _ipaper ipaper
`;
  }

  if (normalized === 'fish') {
    return `# Fish completion for ipaper tunnel
# Save to ~/.config/fish/completions/ipaper.fish

complete -c ipaper -n '__fish_use_subcommand' -a 'serve' -d 'Start the web server'
complete -c ipaper -n '__fish_seen_subcommand_from serve' -l foreground -d 'Run in foreground (for systemd/process managers)'
complete -c ipaper -n '__fish_seen_subcommand_from serve' -l no-daemon -d 'Run in foreground (alias for --foreground)'
complete -c ipaper -n '__fish_use_subcommand' -a 'stop' -d 'Stop running instance(s)'
complete -c ipaper -n '__fish_use_subcommand' -a 'restart' -d 'Stop and start the server'
complete -c ipaper -n '__fish_use_subcommand' -a 'status' -d 'Show server status'
complete -c ipaper -n '__fish_use_subcommand' -a 'tunnel' -d 'Tunnel lifecycle commands'
complete -c ipaper -n '__fish_use_subcommand' -a 'logs' -d 'Tail logs'
complete -c ipaper -n '__fish_use_subcommand' -a 'update' -d 'Check for updates'

complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'help' -d 'Show tunnel help'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'providers' -d 'Show providers'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'ready' -d 'Check readiness'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'doctor' -d 'Run diagnostics'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'status' -d 'Show tunnel status'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'start' -d 'Start a tunnel'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'stop' -d 'Stop tunnel'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'profile' -d 'Manage profiles'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'completion' -d 'Generate completions'

complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l provider -d 'Provider id'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l mode -d 'Tunnel mode'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l profile -d 'Profile name'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l config -d 'Config path'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token -d 'Token'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-file -d 'Token file path'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-stdin -d 'Read token from stdin'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l hostname -d 'Hostname'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l dry-run -d 'Validate without applying'
complete -c ipaper -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l qr -d 'Show QR code'
`;
  }
...
function getDataDir() {
  if (typeof process.env.IPAPER_DATA_DIR === 'string' && process.env.IPAPER_DATA_DIR.trim().length > 0) {
    return path.resolve(process.env.IPAPER_DATA_DIR.trim());
  }
...
function getLogFilePath(port) {
  return path.join(getLogsDir(), `ipaper-${port}.log`);
}
...
function resolveProfileByName(profiles, profileName, provider) {
  ...
  if (matches.length === 0) {
    return { profile: null, error: `No tunnel profile found for name '${profileName}'. Run 'ipaper tunnel profile list'.` };
  }
  if (matches.length > 1) {
...
async function resolveAvailablePort(desiredPort, explicitPort = false, onNotice) {
  ...
  if (occupant?.runtime === 'desktop') {
    message = `Port ${startPort} is used by IPaper Desktop; using a free port`;
  } else if (occupant?.runtime) {
    message = `Port ${startPort} is used by an existing IPaper instance; using a free port`;
  } else {
...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `ipaper-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `ipaper-${port}.json`);
}
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `ipaper serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`ipaper serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `ipaper serve`.',
    });
  }
...
async function discoverRunningInstances() {
  ...
  try {
    ...
    const pidFiles = files.filter((file) => file.startsWith('ipaper-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('ipaper-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid || !isProcessRunning(pid)) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `ipaper-${port}.json`));
        continue;
      }
      const instanceFilePath = path.join(runDir, `ipaper-${port}.json`);
      let mtime = 0;
      let startedAt = 0;
      try {
...
async function fetchTunnelProvidersFromPort(port, fetchImpl = globalThis.fetch) {
  ...
  try {
    const response = await fetchImpl(buildLocalUrl(port, '/api/ipaper/tunnel/providers'));
...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running IPaper instance found. Start one with `ipaper serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by IPaper Desktop app. Tunnel attach requires a CLI instance from \`ipaper serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable IPaper tunnel instance. Ensure it is healthy and running IPaper CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running IPaper instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable IPaper instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only IPaper Desktop instance(s) detected. Tunnel attach requires a CLI instance from `ipaper serve`.');
    }

    throw new Error('No attachable IPaper instance found. Start one with `ipaper serve`.');
  }
  ...
  throw new Error(`Multiple IPaper instances found: ${ports}. Use --port <port> or --all.`);
}
...
async function handleTunnelProfileSubcommand(options, action) {
  ...
  if (!sub) {
    ...
    if (!isQuietMode(options)) {
      ...
      clackLog.step('List profiles: `ipaper tunnel profile list`');
      clackLog.step('Show one profile: `ipaper tunnel profile show --name <name>`');
      clackLog.step('Add profile: `ipaper tunnel profile add --provider cloudflare --mode managed-remote --name <name> --hostname <host> --token <token>`');
      clackLog.step('Remove profile: `ipaper tunnel profile remove --name <name>`');
  ...
  if (sub === 'add') {
    ...
    logStatus('info', '[START_PROFILE]', `ipaper tunnel start --profile ${added.name}`);
    clackOutro('');
    return;
  }
  ...
  throw new TunnelCliError(
    `Unknown tunnel profile subcommand '${sub}'.${hint} Use 'ipaper tunnel help'.`,
    EXIT_CODE.USAGE_ERROR
  );
}
...
const commands = {
  async serve(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'IPaper serve' });
    }
    ...
    if (targetPort !== 0) {
      ...
      if (existingPid && isProcessRunning(existingPid)) {
        throw new Error(`IPaper is already running on port ${targetPort} (PID: ${existingPid})`);
      }
      ...
      if (explicitPort && !(await isPortAvailable(targetPort, options.host))) {
        ...
        if (systemInfo?.runtime === 'desktop') {
          throw new Error(
            `Port ${targetPort} is used by IPaper Desktop app. Choose another port or stop the desktop app.`
          );
        }
        if (systemInfo?.runtime) {
          throw new Error(`IPaper is already running on port ${targetPort}. Use \`ipaper status\` or \`ipaper stop --port ${targetPort}\`.`);
        }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      const warningLine = 'IPAPER_UI_PASSWORD is not set';
      const warningDetail = 'browser UI is unsecured. Use --ui-password or IPAPER_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.IPAPER_UI_PASSWORD = effectiveUiPassword;
      }
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting IPaper on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        IPAPER_PORT: String(targetPort),
        OPENCODE_BINARY: opencodeBinary,
        ...(effectiveHost ? { IPAPER_HOST: effectiveHost } : {}),
        ...(effectiveUiPassword ? { IPAPER_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { IPAPER_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting IPaper on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    const resolvedPort = await new Promise((resolve) => {
      ...
      child.on('message', (msg) => {
        if (settled) return;
        if (msg && msg.type === 'ipaper:ready' && typeof msg.port === 'number') {
          settled = true;
          clearTimeout(timeout);
          resolve(msg.port);
        }
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start IPaper');
    ...
    const serveResult = {
      ...
      logs: `ipaper logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
  ...
  async stop(options) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running IPaper instances found');
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (systemInfo?.runtime === 'desktop') {
          ...
          if (isJsonMode(options)) {
            printJson({ stoppedCount: 0, results: jsonResults, messages: [{ level: 'warning', code: 'DESKTOP_MANAGED_PORT', message: `Port ${options.port} is managed by IPaper Desktop and cannot be stopped with this command.` }] });
          }
          if (showOutput) {
            logStatus('warning', `port ${options.port} is managed by IPaper Desktop`, 'cannot be stopped with this command');
        ...
        if (systemInfo?.runtime) {
          ...
          if (showOutput && !unmanagedStopSpin) {
            logStatus('info', `found unmanaged IPaper instance on port ${options.port}`, 'attempting shutdown');
          }
          unmanagedStopSpin?.start(`Stopping unmanaged IPaper on port ${options.port}...`);
          ...
          if (stopped) {
            unmanagedStopSpin?.stop(`Stopped unmanaged IPaper on port ${options.port}`);
            jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
            if (isJsonMode(options)) {
              printJson({ stoppedCount: 1, results: jsonResults });
            }
            if (showOutput && !unmanagedStopSpin) {
              logStatus('success', `stopped IPaper on port ${options.port}`);
            ...
            unmanagedStopSpin?.error(`Could not stop IPaper on port ${options.port}`);
            ...
            if (isJsonMode(options)) {
              printJson({
                ...
                stoppedCount: 0,
                results: jsonResults,
                messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop IPaper on port ${options.port}.` }],
              });
            }
            if (showOutput && !unmanagedStopSpin) {
              logStatus('error', `could not stop IPaper on port ${options.port}`);
              finish('failed');
            }
            printQuietStopResults();
          }
        ...
        if (showOutput) {
          logStatus('info', `no IPaper instance found on port ${options.port}`);
          finish('nothing to stop');
        }
        printQuietStopResults();
        return;
    ...
    for (const instance of runningInstances) {
      ...
      stopSpin?.start(`Stopping IPaper on port ${instance.port}...`);
      try {
  ...
  async restart(options) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running IPaper instances to restart');
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('warning', `no IPaper instance found on port ${options.port}`);
          clackOutro('nothing to restart');
        } else if (isQuietMode(options)) {
          process.stdout.write('restarted 0\n');
        }
    ...
    for (const instance of runningInstances) {
      ...
      restartSpin?.start(`Restarting IPaper on port ${instance.port}...`);
      try {
        await this.stop({
          explicitPort: true,
          port: instance.port,
        ...
        restartSpin?.stop(`Restarted IPaper on port ${restartedPort}`);
        if (showOutput && !restartSpin) {
          logStatus('success', `port ${restartedPort} restarted`, `mode: ${launchMode}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        restartSpin?.error(`Failed to restart IPaper on port ${instance.port}`);
        if (showOutput && !restartSpin) {
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/ipaper/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/ipaper/tunnel/status');
            if (!response.ok) {
      ...
      case 'doctor': {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/ipaper/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
        ...
        if (desktopUnavailablePorts.length > 0) {
          clackLog.message('Only CLI instances (ipaper serve) support tunneling.');
        }

        if (cliPorts.length === 0 && unavailablePorts.length === 0) {
          logStatus('warning', 'No running instances found', 'Start one with `ipaper serve`.');
          clackOutro('No ports available');
          return;
        }
        if (cliPorts.length === 0) {
          logStatus('warning', 'No CLI instances available for tunneling', 'Start one with `ipaper serve`.');
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active IPaper CLI port.',
                  'Example: `http://127.0.0.1:<port>`',
                  'If CLI picked a different port, update Cloudflare or run `ipaper serve --port <port>`.',
                ],
              });
            }
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered IPaper instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable IPaper CLI instances found on safe ports. Start one with `ipaper serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select IPaper instance port',
        ...
        if (instance?.autoStarted && shouldRenderHumanOutput(options)) {
          logStatus(
            ...
            `logs: ipaper logs -p ${instance.port}`,
          );
        }
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for IPaper on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for IPaper health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('IPaper is still starting');
            throw new Error(
              `IPaper on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
              `Wait another minute, then check health with \`curl -fsS ${buildLocalUrl(instance.port, '/health')}\`. ` +
              `If health is OK, retry tunnel start with \`ipaper tunnel start --port ${instance.port}\`. ` +
              `For diagnostics run \`ipaper logs -p ${instance.port}\`.`
            );
          }
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/ipaper/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/ipaper/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/ipaper\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
            spin?.error('Tunnel start timed out');
            throw new Error(
              `Tunnel start timed out after 60s. cloudflared may still be starting; check with \`ipaper tunnel status --port ${instance.port}\`. Run \`ipaper logs -p ${instance.port}\` for details.`
            );
          }
          spin?.error('Tunnel start failed');
          const message = error instanceof Error ? error.message : String(error);
          throw new Error(`${message} Run \`ipaper logs -p ${instance.port}\` for details.`);
        }
        ...
        if (!response.ok || !body?.ok) {
          ...
          const userError = isCloudflareTimeout
            ...
            : baseError;
          throw new Error(`${userError} Run \`ipaper logs -p ${instance.port}\` for details.`);
        }
        ...
        if (isJsonMode(options)) {
          ...
          const optionalTips = [
            { line: 'Check status', detail: 'ipaper tunnel status' },
            { line: 'Stop tunnel', detail: 'ipaper tunnel stop' },
          ...
          if (!selectedProfile && mode === 'managed-remote' && typeof hostname === 'string' && hostname.trim().length > 0) {
            ...
            optionalTips.push({ line: 'Start from saved profile', detail: 'ipaper tunnel start --profile <name>' });
          }
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/ipaper/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
      ...
      default: {
        ...
        const hint = suggestion ? ` Did you mean '${suggestion}'?` : '';
        throw new TunnelCliError(
          `Unknown tunnel subcommand '${subcommand}'.${hint} Use 'ipaper tunnel help'.`,
          EXIT_CODE.USAGE_ERROR
        );
      }
    }
  ...
  async logs(options) {
    ...
    if (options.all) {
      targets = running;
      if (targets.length === 0) {
        throw new Error('No running IPaper instance found.');
      }
    ...
    if (isJsonMode(options)) {
      if (options.follow) {
        throw new Error('`ipaper logs --json` requires `--no-follow` for deterministic JSON output.');
      }
...
const isCliExecution = isModuleCliExecution(process.argv[1], import.meta.url, fs.realpathSync, 'ipaper');
