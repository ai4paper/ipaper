...
`ipaper` is the product's supported built-in agent preset. It is installed as read-only system-trust and is the default for new sessions. It ships the IPaper-owned `find-the-problem` skill and activates [APaper](https://github.com/ai4paper/apaper-plugin), which contributes the `writing` and `creating-figures` skills and `apaper-mcp` literature-research tools. `/find-the-problem` is model- and user-invocable: it appears in the shared composer skill picker, and the Paper status **Start guided workflow** action submits the same ordinary user prompt to begin live search, human shortlisting, PDF boundary analysis, gap verification, and interest/novelty/feasibility selection. In **Agent presets**, users can duplicate `ipaper` into `<dshHome>/.agent-presets`, customize the copied composition and metadata in its own files, and select that custom preset for later sessions. General and code-agent preset roots remain excluded. The supported persona preserves author intent, separates sourced claims from inference, identifies uncertainty and verification gaps, and prohibits fabricated citations, quotations, bibliographic metadata, data, and results.

## Architecture

The Cordis patch composes the shared DSH host/session/transport services, a complete shared Web client roster, IPaper-owned brand and paper-status clients, and the academic preset. It mounts the singleton `paperProjects` host service after DSH storage and workspace services. The service owns the versioned `ipaper_project` domain, one durable graph per workspace, trusted session/parent-lineage resolution, deterministic IDs, optimistic versions, per-workspace write queues, recovery markers, committed snapshots, dynamic process lenses, and paginated graph/history views. A trusted read-only Connection RPC powers a full **Paper status** surface that replaces the conversation viewport while selected. Its default **Find a question** view derives a provenance-first boundary map from the durable graph: research questions are connected to the papers, authors, methods, contributions, and limitations that address them; live-search provenance and shortlist counts stay visible; unverified gaps remain pending; and candidates are compared through interest, novelty, and feasibility gates. A single Paper status button in the left sidebar—or the `Mod+Shift+P` shortcut—toggles between that surface and chat; framing, research, argument, production, and validation signals are shown without inventing a global completion percentage. The `ipaper` preset exposes exactly `ipaper_record` and `ipaper_state`, with a scoped system-prompt protocol for root and subagent use. Host registry identities remain product-neutral; product wording is confined to user-facing CLI text, prompts, metadata, branding, and preset content.
